...
class HybridRetriever:
    ...
    async def search(
        ...
        # 「広く取って絞る」第1段として top_k の 3 倍を取得する
        fetch_k = top_k * 3
        ...
        if not hybrid or self.bm25.count == 0:
            ...
            results = []
            for chunk_id, score in fused_ids[:top_k]:
                text = chunk_texts.get(chunk_id, "")
                if not text:
                    text = self.vector_store.load_chunk(chunk_id)
                results.append((chunk_id, score, text))
