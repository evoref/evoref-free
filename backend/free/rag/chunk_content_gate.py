"""取得直後の chunk 内容精査ゲート (heuristics-first + 境界 aux)
...
prose チャンクだけ補助タスクで 1 回だけ関連性を判定する
(``JudgeUsageTracker`` で発火上限を抑制)。

degraded mode (``aux_client=None``) / cap 超過 / error / timeout では
ヒューリスティックのみで確定し、例外は決して投げない (応答パスを止めない)。

chat mode では近似重複除去のみを行い、relevance floor / コードシグナル /
aux は適用しない (チャットの recall を退行させないため)。
"""
...
# marginal band の aux 判定で各 chunk を切り詰める文字数 (self_rag_judge と同等)。
_TRUNCATE = 150
...
def _parse_relevant_indices(res: object, count: int) -> set[int] | None:
    """aux 応答から有効な relevant_indices 集合を取り出す。
...
class GateConfig:
    ...
    def from_rag_cfg(cls, rag_cfg: dict | None) -> GateConfig:
        ...
        return cls(
            ...
            judge_enabled=bool(cfg.get("judge_enabled", True)),
...
class ChunkContentGate:
        ...
        *,
        ...
        try:
            result = await self._run(
                query, merged, mode, aux_client, tracker, session_id,
            )
        ...
        if self._debug_logger is not None:
            self._debug_logger.log_content_gate(
                ...
                judge_dropped=result.judge_dropped,
                judge_skipped_reason=result.judge_skipped_reason,
            )
        return result.kept

    async def _run(
        self, query, merged, mode, aux_client, tracker, session_id,
    ) -> GateResult:
        cfg = self._cfg
        ...
        #     prose-only marginal は aux 判定対象
        ...
        # (d) marginal prose を境界 aux で判定
        kept_prose_idx = await self._judge_marginal(
            query, prose_idx, deduped, aux_client, tracker, session_id, result,
        )
    ...
    async def _judge_marginal(
        ...
        """prose-only marginal チャンクを LLM で関連判定し、keep する deduped index 集合を返す。

        判定無効 / 未接続 / cap 超過 / timeout / error / 不正応答では
        全 prose を keep する (heuristic-only, 安全側)。
        """
        cfg = self._cfg
        if not prose_idx:
            return set()
        if not cfg.judge_enabled or aux_client is None:
            result.judge_skipped_reason = (
                "judge_disabled" if not cfg.judge_enabled else "judge_unavailable"
            )
        ...
        if tracker is not None:
            ...
            if not decision.allowed:
                result.judge_skipped_reason = decision.reason
                return set(prose_idx)
        ...
        try:
            ...
            res = await aux_client.generate_json(
                prompt, max_tokens=64, temperature=0.1,
                purpose="retrieval_chunk_gate", list_key="relevant_indices",
                timeout=aux_client.resolve_effective_timeout("retrieval_chunk_gate"),
            )
        except (TimeoutError, asyncio.TimeoutError):
            result.judge_skipped_reason = "timeout"
            return set(prose_idx)
        except Exception as e:
            logger.debug("content gate aux failed: %s", e)
            result.judge_skipped_reason = "error"
            return set(prose_idx)

        relevant_local = _parse_relevant_indices(res, len(prose_idx))
        if relevant_local is None:
            result.judge_skipped_reason = "invalid_response"
            return set(prose_idx)
        if tracker is not None:
            tracker.record(session_id, namespace="content_gate")
        result.judge_used = True
        kept = {prose_idx[n] for n in relevant_local}
        result.judge_dropped = len(prose_idx) - len(kept)
        return kept
