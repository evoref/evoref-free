...
#:   (``chat_service._TRUNCATED_HISTORY_GUIDANCE`` の指示でモデルが末尾に足す)。
#:   行末まで。**``※`` は無いことの方が多い** — 指示文は鉤括弧の文言しか示さず、
#:   モデルは「会話の前半は参照できないため、これより前にもあった可能性が
#:   あります。」と素の文で足す。``※`` を要求していた間はこの形が落ちず、
#:   履歴 → 参考情報 → 次の応答 → few-shot → eval_core の期待キーワードまで
#:   伝播した (2026-09-07 ライブ監査 F-07 / F-01)。
#:
...
#: 謝罪の前置き (「すみません、住まいを間違えました」) も談話標識と同じ扱い —
#: 訂正の冒頭に置かれるだけで値を運ばない (2026-09-09 ライブ監査 P-5)。
...
_REQUEST_ENDING_RE = re.compile(
    ...
    # 活用語を挟まない。
    #
    # 空白を許すのは、数量詞を分かち書きする書き方 (「〜を **2 文で**。」
    # 「その 1 点を **英語 1 文で**。」) が実際に多いため。空白を含めていな
    ...
    # 簡潔に) が続いて ``で`` / ``に`` で終わる。上の規則はひらがなを許さない
    # ので「それぞれ」で外れ、「私の名前と職業と住んでいる都市を、それぞれ
    # 一言で。」が name / occupation / location の 3 スロットに **裸の目的語
    # 句** として立った (2026-09-10 ライブ監査 (i) I-09)。
    r"|を[、，,]\s*[^。．.！!？?、，,]{0,16}?[でに]"
    r"[。．.！!\s\"'」』）)]*\s*$"
...
#: 文を閉じる末尾 (句読点 / 括弧 / コード / 表)。
_TERMINAL_TAIL_RE = re.compile(r"[。．.!！?？」』）)\]｝}>*`|]\s*$")
#: 日本語の文末に立てる仮名 (終止形・助動詞の末尾・終助詞)。これ以外の仮名で
#: 終わる短い応答は、語の途中か連用形で切れている。
#: ``は`` / ``で`` / ``も`` / ``と`` は挨拶 (こんにちは) や体言止めの言い切り
#: (途中まで) に立つので文末扱い。切れた断片で多いのは連用形 (あり / し / き)。
_JA_SENTENCE_FINAL_KANA = frozenset("いえうんすたるかねよぞぜなわさっはでもと")
#: 「短い」の上限。これより長い応答は箇条書きや表で句点を持たないことが普通。
_CUT_OFF_MAX_CHARS = 12


def is_cut_off_answer(text: str) -> bool:
    """応答が **語の途中で切れた** 短い断片か (純粋関数)。

    実インシデント (2026-09-11 ライブ監査 (j) J-07): 「この会話で私が誤って
    主張したことはありましたか。」への応答が「はい、あり」の 5 文字で終わった
    (finish_reason は length ではない — モデルが途中で停止トークンを出した)。
    ``[要約]`` や字数の検証は「長すぎる」側しか見ておらず、切れた断片は
    そのまま返って few-shot / 経験にも「成功」として残る。

    語彙ではなく形で決める: 短く (``_CUT_OFF_MAX_CHARS`` 以下)、文を閉じる
    記号で終わらず、末尾の仮名が文末に立てるもの (い/え/う/ん/す/た/る…) では
    ない。「コーヒー」「4128」「紺色」のような一語の答えは仮名で終わらないので
    掛からず、「はい」「いいえ」「ください」は文末の仮名なので掛からない。
    """
    stripped = (text or "").strip()
    if not stripped or len(stripped) > _CUT_OFF_MAX_CHARS:
        return False
    if _TERMINAL_TAIL_RE.search(stripped):
        return False
    last = stripped[-1]
    if not ("ぁ" <= last <= "ん"):
        return False
    return last not in _JA_SENTENCE_FINAL_KANA
...
def match_line_count(text: str) -> int:
    ...
    **他人の行数も指定ではない。** 「2つ目の基準は、関数の行数を50行以内に
    ...
    for m in LINE_COUNT_RE.finditer(src):
        ...
        # 「1 行ずつ計 4 行で」の「1 行ずつ」も配分で、指定は後ろの「4 行」
        # (2026-09-07 ライブ監査: 4 行で正しく答えた応答に「1 行の指定に対し、
        # 上の回答は 4 行です」という偽の注記が付いた)。
        tail = src[m.end():m.end() + 12]
        if tail.startswith(("は", "あたり", "当たり", "につき", "ずつ")):
            continue
