...
# ─────────────────────────────────────────────────────────────────────
#
# 意図判定は「発話のどこかに動詞があるか」で決めてはいけない。**何をして
# ほしいか** は依頼している文にしか書かれておらず、それ以外の文は状況説明・
# バグの描写・引用でありうる。
#
# 実インシデント (2026-09-06 監査 F-02): 「…tail を追い越してデータを
# **上書きします**。… **修正版**のリングバッファを示してください。」に対し、
# ルータは説明文の「上書き」から書込み動詞と書込み先の双方を拾い
# ``local_write_intent`` と判定した。依頼文には書込み要素が 1 つも無い。
# 結果、成立しないファイル出力タスクが組まれ、26.6 分かけて生成した成果物を
# 捨てて「(書き込みが実行されませんでした)」だけを返した。
#
# 文末が依頼形の文だけを残せば、説明文の語彙は意図判定に入らない。依頼形の
# 文が 1 つも無い発話 (「E:\\tmp\\a.txt に保存」のような体言止め) は判定不能
# なので全文へ倒す — 絞り込みは **依頼文が明示されている発話にだけ** 効かせ、
# 既存の検出を狭めない。

#: 文の区切り。日本語の句点・感嘆・疑問・改行、および英語のピリオド類。
#:
#: **数字の直後のピリオドは文末にしない。** 「4. 次の予定」のような番号付き
#: 見出しは文の途中に現れ、そこで切ると **依頼の前半 (宛先を含む節) が捨て
#: られる**。実インシデント (2026-09-16): 「E:\\tmp\\report.docx に
#: 「4. 次の予定」という節を追加してください。」が
#: ``次の予定」という節を追加してください。`` まで削られ、パスが消えて
#: ``write_destination_evidence`` が偽になり、書込み依頼として成立しなかった
#: (層は deliberative へ落ち、モデルは「追加しました」と報告したのに
#: ファイルは 1 バイトも変わっていない)。
#:
#: 否定戻り読みは **2 文字** で書く。``(?<![0-9])(?<=\.)`` は区切り位置の直前
#: 1 文字 (``.`` 自身) を見てしまい、数字は一度も検査されない。
...
#: 文末が依頼・命令であることの印。
#:
#: - ``ください`` / ``お願い`` / ``してほしい`` / ``せよ`` — 明示的な依頼
#: - 連用テ形の言い切り (「示して」「保存して」) — 口語の依頼
#: - 英語の命令形は文頭で見る (``please write ...`` / ``show me ...``)
_REQUEST_TAIL_RE = re.compile(
    r"(?:ください|下さい|くださいませ"
    r"|お願い(?:します|いたします|できます)?"
    r"|して(?:ほしい|欲しい|くれ|もらえ|いただけ)\w*"
    r"|(?:せよ|しろ|しなさい)"
...
_REQUEST_HEAD_EN_RE = re.compile(
    r"^\s*(?:please\s+)?(?:write|save|create|append|export|output|show|give|"
    r"generate|make|add|update|fix|refactor|implement)\b",
    re.IGNORECASE,
)
...
def request_clauses(text: str) -> str:
    ...
    依頼形の文が 1 つも見つからなければ元の発話をそのまま返す (判定不能な
...
def refers_to_ongoing_session(query: str) -> bool:
    """進行中の会話そのものを指す前置き (「この会話で」「ここまでの会話」) を含むか。

    :data:`_SESSION_ANCHOR_ANY_RE` (話題切断の否定先読み込み) を router の
    層振り分けにも使う。会話への自己参照は答えが窓の中にある想起で、
    ツール計画 (meta_cognitive) に回すと現在セッションを索引で探し回る
    (2026-09-10 ライブ監査 (i) I-08: 「この会話の計算結果をすべて表にして
    ください」が「計算」で tool_patterns → search_history 3 回 + calculate、
    171 秒、``python`` フェンス付きの表)。
    """
    return bool(query) and bool(_SESSION_ANCHOR_ANY_RE.search(query))


def session_position_kind(query: str, *, anchored: bool = False) -> str | None:
    ...
    ``anchored`` は呼出側が **会話の文脈から** アンカーを確定した印。
    「最初に私が聞いたのは何月の予定でしたか」は「この会話で」を言わないが、
    先行ターンのあるセッションで会話内の位置だけを指す想起
    (:func:`only_session_ordinal_recall`) は進行中の会話を指す — ツール判定の
    ガードも同じ根拠で除外検索を抑止している。そこで抑止だけして事実を
    渡さないと「確認できていません」に落ちる (2026-09-10 ライブ監査 (f) F-11)。
    ...
    if not query:
        return None
    if not anchored and not _SESSION_ANCHOR_ANY_RE.search(query):
        return None
...
_WRITE_PROHIBITION_RE = re.compile(
    ...
    # 五段の保存動詞の未然形 + 否定 (書き込ま / 書き出さ / 書き足さ / 書出さ)。
    # 「書き込まず」だけを書いていたので「ファイルには書き出さず」が素通りし、
    # 保存先を尋ねる注記が付いた (2026-09-09 ライブ監査 (e) E-01)。否定は
    # 語彙ではなく活用形 (未然形 + ず / ない) で捕まえる。
    r"|(?:書き込|書込|書き出|書出|書き足|書き加)[まさえ](?:ず|ない|なくて)(?!と)"
    # ファイル語 + 一般動詞 + 否定
...
#: 「覚えておいて」型の **保存指示**。想起の問い (「覚えていますか」) ではなく、
#: いま述べた (または直前の) 内容を記憶に留めるよう頼む形。ツールは要らず、
#: 受け取った内容を言い換えて確認するのが正しい応答。
_MEMORIZE_REQUEST_RE = re.compile(
    r"(?:覚え|おぼえ)(?:て|と)(?:お|い)?(?:いて|て|いてね|きて)?"
    r"(?:ください|下さい|くれ|ほしい|欲しい|ね)?(?![いま])"
    r"|(?:記憶|メモ|保存|記録)し(?:て|と)(?:お|い)?(?:いて|て)?(?:ください|下さい|くれ|ほしい)?"
    r"|忘れない(?:で|よう|でください|でね)|忘れずに"
    r"|(?<![A-Za-z])(?:please\s+)?remember\s+(?:this|that|it)(?![A-Za-z])"
    r"|(?<![A-Za-z])(?:keep\s+(?:this|that|it)\s+in\s+mind|make\s+a\s+note|take\s+note)(?![A-Za-z])",
    re.IGNORECASE,
)
#: 想起の問いの活用 (「覚えていますか」「覚えてる？」) は保存指示ではない。
_MEMORIZE_RECALL_FORM_RE = re.compile(
    r"(?:覚え|おぼえ)て(?:い|る|ま)|(?<![A-Za-z])do\s+you\s+remember(?![A-Za-z])",
    re.IGNORECASE,
)


def memorize_request(query: str) -> bool:
    """クエリが「覚えておいて」型の保存指示か (純粋関数)。

    保存指示にツールは要らない (記憶側の pin 検出が同じ語で発火する)。
    ``tool_patterns`` (計算 / 結果 …) で meta_cognitive へ流れると、search_history
    の空振り文字列がそのまま返答になった (2026-09-10 ライブ監査 (g) G-03:
    「ポンドの計算結果を覚えておいてください」→「No results found for: …」)。
    """
    text = query or ""
    if not text or _MEMORIZE_RECALL_FORM_RE.search(text):
        return False
    return bool(_MEMORIZE_REQUEST_RE.search(text))
...
def persist_request(query: str) -> bool:
    ...
    書込みの **禁止** (:func:`write_prohibited`) が明示されていれば保存依頼では
    ない。「ファイルには保存せず、画面に表示するだけで」の「保存」だけを見て
    persist_intent に分類し、保存先を尋ねる注記 (deliberative の
    write-target-unknown) が付いた — ルータ側 (``write_intent_probe``) は
    2026-09-09 監査 H-04 で禁止を見るようになったが、この読み手だけが
    素通ししていた (2026-09-09 ライブ監査 (d) D-02)。同じ判定の読み手は
    全員この 1 本を通る。
    """
    if not query or write_prohibited(query):
        return False
    return bool(_PERSIST_REQUEST_RE.search(query))
...
#: メモリ / 永続メモリ」の 3 種と答えた。実装は作業記憶 / エピソード記憶 /
#: 意味記憶の 3 層 (c_16。当時は WM / STM / LTM + SemMem) で、
...
_STATEMENT_TAIL_RE = re.compile(
    ...
    # 否定の申告 (「私はペットを飼っていません。」) も平叙の自己申告。
    # 欠けていたため、否定形の申告 → 想起質問の並びが言い直しに数えられた
    # (2026-09-05)。
    r"|ません|ませんでした|ではありません|じゃありません|ではない|じゃない)"
    r"\s*[。．.!！]*\s*$",
)
...
#: 挨拶・感謝・相槌・労い・謝罪の定型。**全文がこれだけで埋まるか** を見る
#: ためだけの語彙で、部分一致では使わない (:func:`is_contentless_social_formula`)。
#:
...
_SOCIAL_FORMULA_RE = re.compile(
    ...
    r"よろしくです|こちらこそ|"
    # 感謝
    r"ありがとうございました|ありがとうございます|ありがとう|"
    r"どうも(?:ありがとう)?|感謝します|助かりました|助かります|"
    r"どういたしまして|恐れ入ります|恐縮です|"
    # 了承・相槌
    r"了解(?:しました|いたしました|です)?|承知(?:しました|いたしました|です)?|"
    r"かしこまりました|(?:よく)?(?:分|わ)かりました|"
    r"なるほど(?:ですね)?|そうです(?:ね|か)|そうなんです(?:ね|か)|"
    r"はい|いいえ|ええ|うん|オーケー|オッケー|"
    # 労い
    r"お疲れ(?:様|さま)(?:です|でした)?|ご苦労(?:様|さま)(?:です|でした)?|"
    # 謝罪
    r"申し訳(?:ありません|ございません|ないです)(?:でした)?|"
    ...
    r"(?:何か|なにか|他に|ほかに|その他)|"
    r"(?:ご不明(?:な)?点|ご質問|ご要望|ご相談)|"
    r"お手伝いできること|お役に立てること|"
    r"(?:が|は|も)?(?:あれば|ありましたら|ございましたら|ありますか|"
    r"ございますか|何でしょうか|なんでしょうか)|"
...
_SOCIAL_FORMULA_RE_EN = re.compile(
    r"(?:"
    r"good\s+(?:morning|afternoon|evening|night)|"
    r"hello|hi|hey|"
    r"thank\s+you(?:\s+very\s+much|\s+so\s+much)?|thanks(?:\s+a\s+lot)?|thx|"
    r"you'?re\s+welcome|no\s+problem|np|"
    r"got\s+it|understood|sure|alright|all\s+right|okay|ok|"
    r"sorry|my\s+bad|"
    r"good\s?bye|bye|see\s+you|take\s+care|"
    r"nice\s+to\s+meet\s+you|please"
    r")",
    re.IGNORECASE,
)

#: 定型の間を埋める句読点・記号・空白。
_SOCIAL_SEPARATOR_RE = re.compile(
    r"^[\s　、。，．,.!！?？…‥・ー〜~\-—:：;；\"'“”「」『』()（）]+",
)
...
def is_contentless_social_formula(text: str) -> bool:
    ...
    のように、**命題を 1 つも運ばない** 発話を見分ける。後で想起しても何も
    答えられないので、エピソード記憶のノートにしても枠を食うだけになる。

    実インシデント (2026-09-16 ライブ監査 F-03): リセット直後の 50 ターンで
    episodic の 37 ノート中 9 件がこの形になり、較正が確定するまでの 13 ターンは
    ``[参考情報]`` の 4〜5 枠が **全部これで埋まった** (「1480 円の商品を 9 つ
    買うと合計いくらですか。」に「ありがとうございます。助かりました。」等)。
    しかもこれらは短い日本語の問いに対して cosine 0.47〜0.57 と高い帯に居座る
    ので、**関連度の棒を上げる方向では直らない** — 書き込み側で止めるしかない。

    判定は **全文被覆**。定型と区切り記号だけで文字列を消費し切れたときだけ
    ...
    consumed = False
    while rest:
        sep = _SOCIAL_SEPARATOR_RE.match(rest)
        if sep:
            rest = rest[sep.end():]
            continue
