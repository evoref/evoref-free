...
DOCUMENT_NOUNS_NEEDS_SUFFIX: tuple[str, ...] = (
    ...
    "README", "読本",
    # 「〜メモ」は **複合語だけ** を採る。裸の「メモ」は 2026-07-15 に
    # 「朝礼メモ」で長文生成へ流出した語で、単体では文書生成の signal に
    # ならない (買い物メモ / 伝言メモ のように短文の依頼が普通)。
    #
    # 発見の経緯 (2026-09-15 ライブ監査): 層振り分けの shadow 評価で
    # 「リトライ設計の方針をまとめた設計メモを書いてください。」が
    # 規則 deliberative / 事例 meta_cognitive と食い違った。「設計書」なら
    # long_form へ乗るのに「設計メモ」は文書名詞に無く default に落ちていた
    # (docs/c_17 §3.3 の shadow が拾った最初の実例)。
    "設計メモ", "議事メモ", "作業メモ", "打ち合わせメモ", "打合せメモ",
    "調査メモ", "検討メモ", "技術メモ",
)
