...
async def reload_reranker(state: AppState) -> None:
    """reranker セクション変更時に reranker を再生成して差し替える

    reranker.enabled=false なら NullReranker を注入し、llama-server プロセスも
    停止してメモリを解放する。enabled=true なら落ちていればプロセスを起動し直して
    backend を再構築する (assist と同じ挙動)。
    """
    ...
    if old is not None and hasattr(old, "aclose"):
        try:
            ...
            logger.warning("Failed to close old reranker: %s", e)
    ...
    if not reranker_cfg.get("enabled", False):
        ...
        state.reranker = NullReranker()
        # OFF にしたら llama-server プロセスも停止してメモリを解放する
        _stop_reranker_server_process(state, cfg)
        logger.info(
            "Reranker reloaded: disabled via config (enabled=false) — "
            "using NullReranker (auto SKIP)"
        )
        return

    # ON にしたらプロセスが落ちていれば起動し直す (既に起動済みなら no-op)
    await _start_reranker_server_process(cfg)

    try:
        reranker = await create_reranker_backend_async(
            cfg, debug_logger=state.debug_logger,
        )
        state.reranker = reranker
        logger.info("Reranker reloaded: is_active=%s", reranker.is_active)
    except Exception as e:
        logger.error("Failed to reload reranker: %s", e)
        state.reranker = None


def _stop_reranker_server_process(state: AppState, cfg: dict) -> None:
    """reranker の llama-server プロセスを停止してメモリを解放する
    ...
    try:
        stopped, msg = stop_server_process("reranker", cfg, state)
        logger.info("Reranker server process stop on disable: %s (%s)", stopped, msg)
    except Exception as e:  # noqa: BLE001
        logger.warning("Failed to stop reranker server process on disable: %s", e)


async def _start_reranker_server_process(cfg: dict) -> None:
    """reranker の llama-server プロセスを起動する (既に起動済みなら no-op)

    起動に失敗しても後続の health_check 失敗で LazyReranker フォールバックに
    倒れるため、例外は握りつぶして warning に留める。
    """
    from backend.free.api.system.server_control import start_server_process

    try:
        started, msg = await start_server_process("reranker", cfg)
        logger.info("Reranker server process start on enable: %s (%s)", started, msg)
    except Exception as e:  # noqa: BLE001
        logger.warning("Failed to start reranker server process on enable: %s", e)
