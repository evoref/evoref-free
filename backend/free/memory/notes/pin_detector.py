...
#: 疑問形の文末 (「覚えていますか？」「覚えていますか。」等)。
#: 疑問符だけを見ると「私の名前、覚えていますか。」が想起の依頼なのに pin
#: された (2026-09-02 監査 B-5)。終助詞 ``か`` は活用語尾 (ます / です / る /
#: た …) の直後に限る — 「〜ですから。」の ``か`` や「いくつか。」の名詞末は
#: 疑問ではない。
_PIN_QUESTION_ENDING_RE = re.compile(
    r"(?:[?？]|(?:ます|です|ました|でした|ません|でしょう|だろう|ない|る|た|の)"
    r"か[。．.]?)\s*$",
)
