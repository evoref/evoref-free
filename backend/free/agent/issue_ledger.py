...
KIND_LABELS: dict[str, str] = {
    ...
    # 本文の決定論的な破綻 (``FeedbackCollector._derive_turn_outcome``)。台帳が
    # ツールの例外しか持たないと、計算の破綻や自己矛盾があった会話でも
    # 「不首尾は確認されていません」と自己申告する (2026-09-05 ライブ監査 F-13)。
    "content_contradiction": "回答の中身が検証と食い違った (計算の矛盾 / 自己撤回 / 実測値との不一致)",
    "tool_result_ignored": "ツールの計算結果を回答に使わず、別の数値を述べた",
    "output_broken": "回答の文字列が崩れていた",
}
